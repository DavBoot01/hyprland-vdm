...
inline static const std::array<SHyprCtlCommand, 5> PLUGIN_COMMANDS = {
    {{.name = CMD_DISPATCH_VDMINFO_STR,
      .exact = true,
      .fn = [](eHyprCtlOutputFormat f, std::string a) { return handleDbgPluginInfo(f, a); }},
     {.name = CMD_DISPATCH_VDLIST_STR,
      .exact = true,
      .fn = [](eHyprCtlOutputFormat f, std::string a) { return handleVirtualDesktopList(f, a); }},
     {.name = CMD_DISPATCH_VDLAYOUT_STR,
      .exact = false,
      .fn = [](eHyprCtlOutputFormat f, std::string a) { return handleVirtualDesktopLayout(f, a); }},
     {.name = CMD_DISPATCH_VDINFO_STR,
      .exact = false,
      .fn = [](eHyprCtlOutputFormat f, std::string a) { return handleVirtualDesktopInfo(f, a); }},
     {.name = CMD_DISPATCH_PRINTMONITORINFO_STR,
      .exact = false,
      .fn = [](eHyprCtlOutputFormat f, std::string a) { return handleMonitorInfo(f, a); }}}};
 ...
 */
inline Hyprutils::Memory::CSharedPointer<SHyprCtlCommand>
registerHyprCtlCommand(HANDLE handle, const char *name,
