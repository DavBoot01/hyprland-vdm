...
// ---- Logger facade ----
struct Logger {
    void log(CLI::eLogLevel level, const std::string_view &msg,
...
void initLogging(bool toStdout = true, bool colored = true, const char *filePath = nullptr,
...
inline void logWithCategory(LogLevel lvl, const std::string_view &category,
                            const std::string_view &msg, const std::string_view &tag = "") {
    if (lvl < getMinLevel())
        return;
    const auto tagOpt = tag.empty() ? std::optional<std::string_view>{} : std::make_optional(tag);
    const auto categoryOpt =
        category.empty() ? std::optional<std::string_view>{} : std::make_optional(category);
    getLogger().log(toUnderlying(lvl), msg, tagOpt, categoryOpt);
}

// Convenience helpers per level
inline void logTrace(const std::string_view &msg, const std::string_view &tag = "") {
    log(LogLevel::Trace, msg, tag);
}
inline void logInfo(const std::string_view &msg, const std::string_view &tag = "") {
    log(LogLevel::Info, msg, tag);
}
inline void logWarn(const std::string_view &msg, const std::string_view &tag = "") {
    log(LogLevel::Warn, msg, tag);
}
inline void logError(const std::string_view &msg, const std::string_view &tag = "") {
    log(LogLevel::Error, msg, tag);
}

inline void logTrace(const std::string_view &category, const std::string_view &msg,
                     const std::string_view &tag = "") {
    logWithCategory(LogLevel::Trace, category, msg, tag);
}
inline void logInfo(const std::string_view &category, const std::string_view &msg,
                    const std::string_view &tag = "") {
    logWithCategory(LogLevel::Info, category, msg, tag);
}
inline void logWarn(const std::string_view &category, const std::string_view &msg,
                    const std::string_view &tag = "") {
    logWithCategory(LogLevel::Warn, category, msg, tag);
}
inline void logError(const std::string_view &category, const std::string_view &msg,
                     const std::string_view &tag = "") {
    logWithCategory(LogLevel::Error, category, msg, tag);
}
...
#define APPLOG_WITH_CTX(LVL, MSG)                                                                  \
    AppLog::getLogger().log(AppLog::toUnderlying(LVL), std::string_view{MSG})
