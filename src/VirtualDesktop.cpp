...
namespace VDM {
    ...
    std::optional<CVirtualDesktop::WorkspaceId> CVirtualDesktop::getWorkspaceForMonitor(MonitorId monitorId) const {
    ...
    std::string CVirtualDesktop::toList(bool detailed) const {
        ...
        ss << "Virtual Desktop ID " << m_id << " (" << m_name << ") - Active: " << (m_isActive ? "Yes" : "No");
    ...
    std::string CVirtualDesktop::toJson(bool detailed, bool asNode) const {
        ...
        if (detailed) {
            ...
            std::vector<std::pair<MonitorId, WorkspaceId>> sortedBindings(m_workspaceByMonitor.begin(), m_workspaceByMonitor.end());
            std::sort(sortedBindings.begin(), sortedBindings.end(), [](const auto& a, const auto& b) {
                return a.first < b.first;
            });
