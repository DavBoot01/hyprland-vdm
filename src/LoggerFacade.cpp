...
std::string buildDecoratedMessage(const std::string_view &msg, std::optional<std::string_view> tag,
...
void Logger::log(UnderlyingLevel level, const std::string_view &msg,
...
void setMinLevel(LogLevel lvl) {
    gMinLevel.store(lvl, std::memory_order_relaxed);
}
