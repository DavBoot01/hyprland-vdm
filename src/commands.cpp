...
std::string handleVirtualDesktopInfo(eHyprCtlOutputFormat format, std::string args) {
    ...
    const std::string state =
        vd->toStringDetailed(format == eHyprCtlOutputFormat::FORMAT_JSON, true);
    if (format == eHyprCtlOutputFormat::FORMAT_JSON) {
        nlohmann::json j;
        j["status"] = "ok";
        j["vdeskId"] = vId;
...
std::string handleVirtualDesktopLayout(eHyprCtlOutputFormat format, std::string args) {
    ...
    const std::string layout =
        all ? mgr.getLayout().toStringDetailed(wantJson) : mgr.getLayout().toString(wantJson);
...
Hyprutils::Memory::CSharedPointer<SHyprCtlCommand>
registerHyprCtlCommand(HANDLE handle, const char *name,
    ...
    if (!ptr) {
        ...
        HyprlandAPI::addNotification(handle, ss.str(), CHyprColor(0.8, 0.2, 0.2, 1.0), 5000);
    }
    return ptr;
}
